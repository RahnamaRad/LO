...
eror           =     data.frame(c( rep("3 fold CV", m), rep("5 fold CV", m), rep("7 fold CV", m), rep("LO", m),   rep("outErr", m) ), n*c(lambda, lambda, lambda, lambda, lambda) ,c(cv.3f.mean, cv.5f.mean, cv.10f.mean, cv.lo.mean, extraErr.mean),c(cv.3f.se, cv.5f.se, cv.10f.se, cv.lo.se, extraErr.se))
